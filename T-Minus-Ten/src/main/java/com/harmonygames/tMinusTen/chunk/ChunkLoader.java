...
public class ChunkLoader implements Runnable {
    ...
    public void loadChunks(Chunk... chunks) {
        ...
        if(thread == null) thread = new Thread(this, "_TMinusTen:ChunkLoader_");
        thread.start();
    }
    ...
    public void run() {
        for(int i = 0; i < waitingList.size(); i++) {
            ...
            for(int x = 0; x < chunk.chunkWidth; x++) {
                ...
                if(chunk.chunkY == 0) yOffset = (int) Math.round(Chunk.heightNoiseMap.noise((chunk.chunkX * chunk.chunkWidth) + x) * 10f);
